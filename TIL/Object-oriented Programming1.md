...
+ 호출스택(call stack 또는 execution stack) : 메서드의 작업에 필요한 메모리 공간을 제공. 메서드가 호출되면 호출스택에 호출된 메서드를 위한 메로리가 할당됨 -> 메모리는 메서드가 작업을 수행하는 동안 지역변수(매개변수 포함)들과 연산의 중간결과를 저장 -> 메서드가 작업을 마치면 할당되었던 메모리공간은 반환되어 비어짐
